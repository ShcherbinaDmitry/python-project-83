...
def get_checks():
    ...
    curs.execute('SELECT DISTINCT url_id, created_at, status_code FROM url_checks ORDER BY url_id DESC')
...
def add_check(data):
    ...
    curs.execute(
        ...
        (data['url_id'], data['status_code'], data['h1'], data['title'], data['description'], datetime.now())
    )
    id = curs.fetchone().id
